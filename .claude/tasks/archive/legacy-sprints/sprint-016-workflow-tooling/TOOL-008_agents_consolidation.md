...
# TOOL-008: Consolidate .claude/agents Documentation (scrum-leader files)
...
git worktree add -b feat/agents-consolidation ../tool-008-worktree
cd ../tool-008-worktree

# Verify you're in the correct worktree:
git branch --show-current  # Should show: feat/agents-consolidation
pwd  # Should show: ../tool-008-worktree
...
**Priority**: LOW
**Worktree**: `../tool-008-worktree` (Branch: `feat/agents-consolidation`)
**Estimated**: 1 hour

## Problem Statement
The .claude/agents directory has a duplicated structure that violates the "one file per agent" principle:

**Current Issue**:
- `scrum-leader.md` - Main agent definition
- `scrum-leader-documentation.md` - Extended documentation for same agent

This creates confusion about which file is authoritative and makes maintenance more complex.

**Suggested Solution**:
Consolidate into a single comprehensive `scrum-leader.md` file, or create a subdirectory structure if extensive documentation is truly needed.

**Additional Requirement**:
Ensure all agent documentation uses proper `@` syntax when referencing files that Claude should read, enabling proper file access during agent execution.

## Acceptance Criteria
- [ ] Only one primary file per agent in .claude/agents/
- [ ] All scrum-leader functionality consolidated into single file
- [ ] No information lost during consolidation
- [ ] Clear file structure maintained (one-to-one agent mapping)
- [ ] Any scripts/references updated to point to consolidated file
- [ ] Consistent with improved documentation structure from TOOL-007
- [ ] Clear precedent set for future agent documentation organization
- [ ] All file references use proper `@` syntax for Claude to read referenced materials

## Technical Approach
### Files to Modify
- `.claude/agents/scrum-leader.md` - Consolidate all scrum-leader information
- `.claude/agents/scrum-leader-documentation.md` - Remove (content moved to main file)
- Any scripts that reference the documentation file
